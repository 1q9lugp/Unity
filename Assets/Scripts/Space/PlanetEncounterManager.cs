...
public class PlanetEncounterManager : MonoBehaviour
{
    ...
    // ── First death — one final chance ───────────────────────────────
    IEnumerator JesusReveal()
    {
        Time.timeScale = 0f;
        yield return StartCoroutine(FadeInJesus());
        // Dialogue panel must sit above the Jesus image
        if (_dialogueBg != null) _dialogueBg.transform.SetAsLastSibling();
        yield return StartCoroutine(ShowDlg("JESUS: Ashtar Sheran. You stand at the threshold. I grant thee one final chance.", 0f));
        yield return StartCoroutine(ShowDlg("JESUS: Rise, Commander. Your people still need you.", 0f));
    ...
    IEnumerator ShowGameOver()
    {
        ...
        yield return StartCoroutine(FadeInJesus());
        if (_dialogueBg != null) _dialogueBg.transform.SetAsLastSibling();
        yield return StartCoroutine(ShowDlg("JESUS: Ashtar Sheran. The battle is lost.", 0f));
        yield return StartCoroutine(ShowDlg("JESUS: You have fought with honour. Now choose — return to rest, or rise once more.", 0f));
        yield return StartCoroutine(ShowDlg("ASHTAR: I will not abandon them. Not now. Not ever.", 0f));
        CloseDlg();
        // Show Retry / Main Menu buttons over the Jesus background
    ...
    IEnumerator ShowDlg(string msg, float hold)
    {
        ...
        {
            bool isA=msg.StartsWith("ASHTAR:"); bool isP=msg.StartsWith("PTAAH:");
            bool isL=msg.StartsWith("LIZARD:"); bool isJ=msg.StartsWith("JESUS:");
    ...
    void BuildRuntimeUI()
    {
        ...
        // Speaker icon (inside dialogue panel)
        ...
        // Announce text (centre screen — used by Panel())
        ...
        // ── Game Over buttons (shown after game over dialogue) ────────
    ...
    // 2. Reactivate Player and Reset State
    if (player != null) {
        player.gameObject.SetActive(true);
        // If your SharaShipController has a Reset or Respawn method, call it here.
        // Also ensure the SpriteRenderer is enabled:
