        ...
        if (firing)
        {
            ...
            if (_fireCd <= 0f)
            {
                ...
                if (beamPrefab != null)
                    ...
                    _sacrificeCount += 1;
                    if (_sacrificeTxt != null) _sacrificeTxt.text = "HUMANS SACRIFICED: " + _sacrificeCount;
            }
        }
        else { _fireCd = 0f; }

        // ── Ashtar Beam recharge + fire ───────────────────────────────
        if (_beamCd > 0f) { _beamCd -= Time.deltaTime; if (_beamCd < 0f) _beamCd = 0f; }
            ...
            if (ashtarBeamPrefab != null)
{
    ...
    IEnumerator HitSeq()
    {
        ...
        for (float t = 0f; t < invincibilityDuration; t += 0.2f)
        {
